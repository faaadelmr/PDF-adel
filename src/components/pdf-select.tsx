...
export default function PdfSelect() {
  ...
  const handleDownloadMerged = async () => {
    ...
    try {
        ...
        if (!pdfBytes) throw new Error("Failed to create PDF.");
        
        const blob = new Blob([pdfBytes], { type: 'application/pdf' });
        const originalFileName = currentFile.name.replace(/\.pdf$/i, '');
        downloadBlob(blob, `${originalFileName}_adelSelected.pdf`);
  ...
  const handleDownloadSplit = async () => {
    if (!currentFile || splitPoints.size === 0) {
      toast({ variant: "destructive", title: "No Split Points", description: "Click between pages to create a split point." });
      return;
    }
    setIsProcessing(true);
    const originalFileName = currentFile.name.replace(/\.pdf$/i, '');
    ...
    try {
        ...
        if (chunks.length === 0 || chunks.every(c => c.length === 0)) {
            toast({ variant: "destructive", title: "Nothing to split", description: "No selected pages within the split ranges." });
            setIsProcessing(false);
            return;
        }
  ...
  return (
    <div>
        <div className="flex flex-wrap gap-3 justify-start items-center mb-6">
            <Button onClick={handleInvertSelection} variant="secondary" size="sm" disabled={isProcessing}><FlipHorizontal className="mr-2"/>Invert Selection</Button>
            ...
            <Button onClick={handleDownloadSplit} variant="outline" size="sm" disabled={isProcessing || splitPoints.size === 0}>{isProcessing ? <Loader2 className="mr-2 animate-spin"/> : <Scissors className="mr-2"/>}Download Split PDFs</Button>
                        ...
                        {!isLastInOrder && (
                            <div className="flex-shrink-0 w-8 h-full flex items-center justify-center -ml-1 -mr-1 z-10">
                                <button
                                    ...
                                    title={splitPoints.has(page.id) ? "Remove split point" : "Add split point"}
                                >
