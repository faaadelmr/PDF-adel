...
export default function PdfEditor() {
    ...
    if (pageGridRef.current) {
      new Sortable(pageGridRef.current, {
        ...
        onEnd: (evt) => {
          ...
          const newOrder = Array.from(pageGridRef.current?.children || []).map(
            (child) => parseInt((child as HTMLElement).dataset.pageId || "0")
          );
  ...
  const createPdf = async (pagesToInclude: number[]) => {
    ...
    const pageIndexes = pagesToInclude.map(num => num - 1);
  ...
  const handleDownloadSeparate = async () => {
    ...
    try {
        ...
        setStatusMessage(`Mempersiapkan unduhan ${totalFiles} file...`);
        
        const batchSize = 5;
        for (let i = 0; i < totalFiles; i += batchSize) {
            const batch = orderedSelected.slice(i, i + batchSize);
            setStatusMessage(`Mengunduh batch ${Math.floor(i / batchSize) + 1} dari ${Math.ceil(totalFiles / batchSize)}...`);
            
            await Promise.all(batch.map(async (pageNum) => {
                const pdfBytes = await createPdf([pageNum]);
                if (!pdfBytes) return;
                const blob = new Blob([pdfBytes], { type: 'application/pdf' });
                downloadBlob(blob, `${originalFileName}_Halaman_${pageNum}_TanmaSelected.pdf`);
                await new Promise(resolve => setTimeout(resolve, 200));
            }));

            if (i + batchSize < totalFiles) {
                setStatusMessage('Menunggu sebelum batch berikutnya...');
                await new Promise(resolve => setTimeout(resolve, 1000));
            }
        }
        
        setStatusMessage(`Semua ${totalFiles} file telah diproses!`);
    } catch (error) {
  ...
  return (
    <div>
        <div className="flex flex-wrap gap-3 justify-start items-center mb-6">
            ...
            <Button onClick={resetState} variant="destructive" size="sm" disabled={isProcessing}><Trash2 className="mr-2"/>Hapus Halaman</Button>
            <Button onClick={handleDownloadMerged} size="sm" disabled={isProcessing}>{isProcessing ? <Loader2 className="mr-2 animate-spin"/> : <Download className="mr-2"/>}Unduh Pdf</Button>
            <Button onClick={handleDownloadSeparate} variant="outline" size="sm" disabled={isProcessing}>{isProcessing ? <Loader2 className="mr-2 animate-spin"/> : <FileDown className="mr-2"/>}Unduh Terpisah</Button>
        </div>
        ...
        <div ref={pageGridRef} className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-8 gap-4">
            {pageOrder.map((pageId) => {
                ...
                return (
                    <div key={page.id} data-page-id={page.id} className="relative group cursor-move" onClick={() => togglePageSelection(page.id)}>
